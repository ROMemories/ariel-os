...
pub fn hw_setup(_args: TokenStream, _item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        mod #mod_name {
            ...
            use #riot_rs_crate::arch::peripherals;
...
mod hw_setup {
    ...
    pub fn generate_sensor(riot_rs_crate: &syn::Ident, sensor_setup: &Sensor) -> TokenStream {
        ...
        if let Some(SensorBus::I2c(i2cs)) = sensor_setup.bus() {
            ...
            let i2c_init = quote! {
                let i2c_bus = #riot_rs_crate::#i2c_bus_static.get().unwrap();
                let i2c_dev = #riot_rs_crate::i2c::I2cDevice::new(i2c_bus);
            ...
            let spi_init = quote! {
                let spi_bus = #riot_rs_crate::#spi_bus_static.get().unwrap();
                // FIXME: depends on the arch (e.g., rp)
                let cs_output = #riot_rs_crate::gpio::Output::new(
                    peripherals.p,
                    #riot_rs_crate::gpio::Level::High,
                    #[cfg(context = "nrf")]
                    #riot_rs_crate::gpio::OutputDrive::Standard,
                );
                let spi_dev = #riot_rs_crate::spi::SpiDevice::new(spi_bus, cs_output);
        ...
        if let Some(peripherals) = sensor_setup.peripherals() {
            ...
            let gpio_init = quote! {
                let pull = #riot_rs_crate::gpio::Pull::#pull_setting;
                let input = #riot_rs_crate::gpio::Input::new(peripherals.p, pull);
        ...
        let (peripheral_struct_path, one_shot_peripheral_struct) = if !peripheral_defs.is_empty() {
            ...
            (
                ...
                quote! {
                    #[cfg(all(#(#cfg_conds),*))]
                    #riot_rs_crate::define_peripherals!(#one_shot_peripheral_struct_ident {
                        #(#peripheral_defs),*
                    });
                },
            )
        ...
        quote! {
            ...
            #[cfg(all(#(#cfg_conds),*))]
            #[#riot_rs_crate::reexports::linkme::distributed_slice(#riot_rs_crate::sensors::SENSOR_REFS)]
