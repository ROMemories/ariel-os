...
Alternatively, there is experimental support for IPv6 configuration through SLAAC,
which can be enabled using the `network-config-ipv6-slaac` [laze module][laze-modules-book].
SLAAC allows generating a link-local address (from the block `fe80::/64`) and (at least) one global address.
However, the SLAAC implementation does not currently support [the Privacy Extensions][ietf-rfc8981]—which allow generating randomized, regularly rotated IPv6 addresses—which means that the global IPv6 address generated by SLAAC is derived from a fixed EUI-64 and is therefore stable across networks, allowing for global tracking of the device.
Additionally, the SLAAC implementation does not currently allow obtaining a list of [recursive DNS servers from Router Advertisements][ietf-rfc8106-rdnss-option] (RA).
Once these shortcomings are resolved,
it will become the default configuration for IPv6.

> [!WARNING]
> Because of the lack of support for the Privacy Extensions, SLAAC should not currently be used in residential deployments because of the risk on privacy.
...
[ietf-rfc8981]: https://www.rfc-editor.org/info/rfc8981/
[ietf-rfc8106-rdnss-option]: https://www.rfc-editor.org/info/rfc8106/#section-5.1
