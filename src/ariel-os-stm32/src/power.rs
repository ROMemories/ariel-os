...
// TODO: use `embassy_stm32::gpio::PinNumber` when bumping `embassy-stm32`.
// Embassy uses `PinNumber` as port number.
type PortNumber = u8;
// TODO: use `embassy_stm32::gpio::PinNumber` when bumping `embassy-stm32`.
type PinNumber = u8;
...
pub fn enter_stop_mode<'a, T: crate::IntoPeripheral<'a, P>, P: StopWakeupPin>(
    ...
    // NOTE(cs): a critical section is used for atomicity.
    let lines = critical_section::with(|cs| {
        ...
        let lines = wait_for_wakeup(cs, trigger);

        p.SCB.clear_sleepdeep();

        // Enables the SysTick interrupt again.
        p.SYST.enable_interrupt();

        // SAFETY: the peripheral is obtained and used entirely in a critical section.
        let mut rcc = unsafe { embassy_stm32::peripherals::RCC::steal() };
        embassy_stm32::rcc::reinit(crate::rcc::CONFIG, &mut rcc);
...
fn configure_exti(trigger: Option<(PortNumber, PinNumber, GpioWakeupTriggerEvent)>) {
...
fn wait_for_wakeup(
    ...
    trigger: Option<(PinNumber, PinNumber, GpioWakeupTriggerEvent)>,
    ...
    loop {
        ...
        // NOTE(cs): This is done inside the critical section to be sure no ISRs can reset the
        // interrupt flags before we read them.
