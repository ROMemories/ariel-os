...
use riot_rs::{self as _, embassy::UserProgram};
...
impl UserProgram for HttpServer {
    fn initialize(
        ...
        let P0_11 = peripherals.P0_11.take().ok_or(())?;
        let button = Input::new(P0_11.degrade(), Pull::Up);
    ...
    fn start(&self, spawner: embassy_executor::Spawner, args: TaskArgs) {
        for id in 0..WEB_TASK_POOL_SIZE {
            let app_state = AppState {
                button: self.button_input,
            };
            spawner
                .spawn(web_task(args, id, self.app, self.config, app_state))
                .unwrap();
        }
    }
}

// TODO: this could be generated by an attribute proc-macro on HttpServer::initialize()
#[linkme::distributed_slice(riot_rs::embassy::EMBASSY_TASKS)]
fn __init_program(
    peripherals: &mut embassy_nrf::OptionalPeripherals,
    args: TaskArgs,
) -> Result<&'static dyn UserProgram, ()> {
    HttpServer::initialize(peripherals, args)
}
